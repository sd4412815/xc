<?php
class MWeiXinEntSystemController extends Controller {
...
// 		Yii::log ( '1ww', CLogger::LEVEL_INFO, 'mngr.mweixinC' );
		...
		$weixinEnt = new UWeChatEnt (AppName::$EntSystemMngr, Yii::app()->request->getParam('nonce'),
...
// 		$weixin->reply ( $reply );
	...
	private function receiveText($weixinEnt){
		...
		$msgContent = strtolower($msgContent);
		$msgContentArray = split('[+ ,:]', $msgContent);
// 		$msgContentArray = explode('+', $msgContent);
		$msgContentFuncCode = @$msgContentArray[0];
// 		$content = $weixinEnt->make
// 		$loginStatus = $this->checkLogin($weChat);
// 		$obj = $weChat->msg;
// 		$content = '';
		switch ($msgContentFuncCode){
			case 'updatemenu':
					$content = $this->createMenu($weixinEnt);
					break;
					case '1':
						$content ="回复序号查看相关内容11： \n\n";
						$content.= "[1]  发放优惠券\n";
						$content.= "[2]  邀请老板关注\n";
					break;
					case '2':
						$bossTel = @$msgContentArray[1];
						$rlt = $this->inviteBoss($weixinEnt, $bossTel);
						Yii::log($rlt,CLogger::LEVEL_INFO,'mngr.mweixinEntSys.inviteBoss');
...
// 				$content = $weixinEnt->makeJsonText($msgContent);
	...
	private function inviteBoss($weixinEntObj,$bossTel, $userId=NULL){
		
		
		if (isset($bossTel)){
			$user = User::model()->findByAttributes(array('u_tel'=>$bossTel));
			if (isset($user)){
				$userId = $user['id'];
			}
		}
// 		$userId = 100;
		if (isset($userId)){
			...
			return $weixinEntObj->inviteUser($userId);
			
// 			$user = User::model()->findByAttributes(array('u_tel'=>$bossTel));
		}else{
			$rlt = array(
					'errcode'=>ErrorCode::$IllegalUserID,
					'errmsg'=>'找不到该成员',
					'type'=>1
			);
			return json_encode($rlt,JSON_UNESCAPED_UNICODE);
		}
	...
	private function createMenu($wexinEntObj) {
		...
		$btnCurrentHui=array('type'=>'click',
				'name'=>'我的优惠',
				'key'=>'currentHui'
		);
		$jsonMenu[]=$btnCurrentHui;
		
		$btnHuiMall=array('type'=>'click',
				'name'=>emoji_softbank_to_unified( UWeChat::utf8_bytes(0x1f389)).'惠商城',
				'key'=>'huiMall'
		);
		$jsonMenu[]=$btnHuiMall;
		
		
		$btnHuiFavorite=array('type'=>'click',
				'name'=>'最受欢迎榜',
				'key'=>'huiFavorite'
		);
		$btnHuiSuggest=array('type'=>'click',
				'name'=>'最推荐榜',
				'key'=>'huiSuggest'
		);
		$btnHuiHistory=array('type'=>'click',
				'name'=>'我的惠记录',
				'key'=>'huiHistoty'
		);
