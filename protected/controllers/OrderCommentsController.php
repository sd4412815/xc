...
class OrderCommentsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','admin','updateComment','disUpdateComment'),
				'users'=>array('@'),
			),
	...
	public function actionUpdateComment($id,$new){
		...
		if ($new ==1) {
			...
			$model['oc_comment_user_id'] = Yii::app()->user->id;
...
// 			$model['oc_comment']='';
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['OrderComments'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('OrderComments');
	...
	{
		$model=new OrderComments('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrderComments']))
			$model->attributes=$_GET['OrderComments'];
	 ...
	 * @return OrderComments the loaded model
	 ...
	 * @param OrderComments $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='order-comments-form')
		{
