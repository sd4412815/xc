...
// 			$cookie_phpsession_id=Yii::app()->request->cookies['PHPSESSID'];
// 			$phpsessid = $cookie_phpsession_id->value;
			$phpsessid = Yii::app()->session->sessionID;
			$userToken = $phpsessid;
			
			$nonce = UTool::randomkeys ( 6 );
			$cookie_nonce = new CHttpCookie('nonce', $nonce);
			Yii::app()->request->cookies['nonce']=$cookie_nonce;
			
			$timestamp = time();
			$cookie_timestamp = new CHttpCookie('timestamp', $timestamp);
			Yii::app()->request->cookies['timestamp']=$cookie_timestamp;
			
			
			$signature = UTool::getSignature(UTool::get_client_ip(),$userToken, $nonce, $timestamp);
			
			$cookie_signature = new CHttpCookie('signature', $signature);
			Yii::app()->request->cookies['signature']=$cookie_signature; 
		}
		...
		$model = new LoginForm();
		$model->setScenario('checkTel');
		if (isset ( $_POST ['ajax'] ) && $_POST ['ajax'] === 'check-tel-form') {
			echo CActiveForm::validate ( $model );
			Yii::app ()->end ();
		}
		
		
		$this->render('CheckTel',array('model'=>$model));
	} 
...
// 		$model['pid'] = City::model()->findByPk($model['cid'])->province['id'];
	...
	private function urlRedirect(){
		...
		$cs->registerScript(rand(0,10000),'top.location="'.Yii::app ()->session ['callback'].'"');
// 		Yii::app ()->session ['urlReferer'];
// 		$this->redirect( Yii::app()->baseUrl ,TRUE);
		$isIframe = Yii::app()->session['iframe'];
		if (isset($isIframe)){
			unset(Yii::app()->session['iframe']);
			$this->redirect(Yii::app()->createUrl('mSite/redirect',array('callback'=>Yii::app()->session['callback'])));
			
		}else
		if (isset(Yii::app()->session['callback'])){
			
			$this->redirect(Yii::app()->session['callback']);
			unset(Yii::app()->session['callback']);
		}else{
...
// 		}
	...
	public function actionRedirect($callback){
		$this->layout="_root";
		$this->render('redirect',array('callback'=>Yii::app()->session['callback']));
	}
...
// 							$weiUser = WeixinOpenid::model()->findByAttributes(array('wo_open_id'=>$openid));
						...
						unset(Yii::app()->session['appServiceId'] );
						unset(Yii::app()->session['srcType'] );
						Yii::app()->user->setFlash('weixinAutologin','您已绑定微信，自动登陆成功');
						$this->urlRedirect();
					} else {
