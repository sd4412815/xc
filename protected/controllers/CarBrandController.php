...
class CarBrandController extends Controller
{
	...
	{
		return array(
			array('allow',  // allow all users to per,form 'index' and 'view' actions
				'actions'=>array('index','view'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','UpdateTypes'),
				'users'=>array('@'),
			),
	...
	public function actionUpdateTypes(){
		if(!is_numeric($_POST['cb'])){
			echo CHtml::tag('option',
					array('value'=>''),'选择车型',true);
				
			Yii::app()->end();
		}
		$data=CarType::model()->findAll('ct_car_brand_id=:brand_id',
				array(':brand_id'=>(int) $_POST['cb']),array('order'=>'ASC ct_spell'));
		if (count($data)<=0) {
			echo CHtml::tag('option',
					array('value'=>''),'选择车型',true);
		...
		$data=CHtml::listData($data,'id','ct_name');
